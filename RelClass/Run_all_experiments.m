...
dataroot = '/vol/share/groups/liacs/scratch/UCR/%1$s/%1$s_%2$s.tsv';
dataname = 'ECG200';
testfile = sprintf(dataroot, dataname, 'TEST');
trainfile = sprintf(dataroot, dataname, 'TRAIN');
...
for counter = 1:length(tau_percent)
    ...
    [early_l, early_l_sct, early_t, final_l, all_l, ts_l, training_time, testing_time, ChebyThresh, final_dims, LDG_necessary] = multi_class_incomplete_classification( data, tau_percent(counter), constraint_type, pred_type, use_LDG, min_d);
...
save(['Results/All_Experiments_Reliable_Early_Results_' constraint_type '_' pred_type], 'results');

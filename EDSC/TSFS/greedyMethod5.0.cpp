...
void main()
{
	...
	int MinK=1;
    int classIndex=4;    // pick the class
	...
	{ 
       ...
       if (index>=0)
       {
		   Feature * currentFeature=fSet.at(index);
			...
			for (unsigned int cf=0;cf<reducedFset.size();cf++)
			{
			   // check if it is redundant with an exsiting feature
				
			   //  xor of the two maps
				bitset<ROWTRAINING> xorResult=(currentFeature->bmap) ^(reducedFset.at(cf)->bmap); 
				if (xorResult.count()==0)
				{
					hasRedundant=true; 
					break;
				}
     ...
     { 
       ...
       if (index>=0)
       {
		   ...
		   // need to remove the redundancy
			...
			bool hasRedundant=false; 
			for (unsigned int cf=0;cf<finalFset.size();cf++)
			{
			   // check if it is redundant with an exsiting feature
				
			   //  xor of the two maps
				bitset<ROWTRAINING> xorResult=(currentFeature->bmap) ^(finalFset.at(cf)->bmap); 
				if (xorResult.count()==0)
				{
					hasRedundant=true; 
					break;

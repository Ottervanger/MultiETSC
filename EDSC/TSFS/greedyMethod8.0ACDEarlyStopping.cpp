...
// early stopping hueristic are added to compute the K+1 candidate matrix when computing K. 
// the program may casue memory problem for large data set
#include <fstream>
#include <iostream>
#include <sstream>
#include <string>
...
double  getSegmentNextValue(int index, int k) ;
//void DisArray(int classIndex, int k, double ** DisArray, int rows, int columns);
//void DisArray(int classIndex, int k, double ** DisArray, int ** EndP, int rows, int columns) ;
void DisArray(int classIndex, int k, double ** DisArray, int ** EndP, int rows, int columns, double ** DisArrayNext, int MaximalK, int MinK) ;
...
Feature * ThresholdLearningAll(int index, double m, int k, int targetClass, double ** DisA, double RecallThreshold, int ** EndP, int alpha);  // this index starting from 1, m is the parameter in the bound, k is the length of feature
...
Feature * ThresholdLearningKDE(int index, int k, int targetClass, double ** DisA, double RecallThreshold, double ProbalityThreshold, int ** EndP, int alpha);
...
{  
	...
	int option=2; // option =1 using thresholdAll, option=2, using the KDE cut
    int MaximalK=DIMENSION/2;  // maximal length
	int MinK=5;
	double boundThrehold=1;  //define the parameter of the Chebyshev's inequality
	double recallThreshold=0; 
	double probablityThreshold=0.1;
	int alpha=3;
   ...
   {  
	   ...
	   clock_t startFeature, endFeature;
		...
		double ** DisANext=NULL;
	   for (k=MinK;k<=MaximalK;k++)  // k starts from 1
	   { // start of outer for
		   double ** DisA;
		   ...
		   if (k==MinK)
		   {   
				DisA=new double *[NumberOfSegments]; 
		   ...
		   else 
		   {
			...
			// make the next length disArray; 
			if (k<MaximalK)
			{
			    int NumberOfSegmentsNext=(DIMENSION-k)*ClassNumber[classIndex]; 
				 DisANext=new double *[NumberOfSegmentsNext]; 
				for (int i=0;i<NumberOfSegmentsNext;i++)
			     {
						DisANext[i]=new double[ROWTRAINING];	
			     }
			}
		...
		if ( !inputFile || !inputFile2)
		{
			...
			DisArray(classIndex, k, DisA, EndP, NumberOfSegments, ROWTRAINING, DisANext, MaximalK, MinK);
			endDisA=clock();
		...
		{  
			...
			if (option==1)
			{  
				temp= ThresholdLearningAll(offset+s, boundThrehold, k, classIndex, DisA, recallThreshold, EndP, alpha);  
				// temp=ThresholdLearningAll(offset+s, boundThrehold, k, classIndex, DisA, recallThreshold);
			}
			else if (option==2)
			{
			  temp=ThresholdLearningKDE(offset+s, k, classIndex, DisA, recallThreshold, probablityThreshold,EndP, alpha);
			}
			else
			{
				cout<<"\n invalide option";
  ...
  classificationAllClassesWithDefault(AllClassesFset, resultFile);
}
...
void PrintFeature(Feature * f)
{
   if (f!=NULL)
   {
    ...
    int row=f->segmentIndex/eachLinebase; 
    int column=(f->segmentIndex % eachLinebase)-1; 
    if (column==-1)
    {
		   row=row-1; 
           column=DIMENSION-f->length;
	}
    cout<<"\n training example index="<<row; // starting from index 0
	cout<<"\n starting position="<< column; // starting position from 0
      ...
      { 
         ...
         if (temp<=currentf->threshold)
         {
            ...
            if (labelTraining[i]==Classes[targetClass])
            {
				...
				weightedRecall=weightedRecall+ pow(((double)1/ EndP[index-classofset][i]),(double)1/alpha);
            }
         }
      }
      ...
      for (int j=0;j<columns;j++)
      {
          ...
          double * segment=getSegment(segmentIndex, k); 
            // compute the best match
		  	// compute the position of the new value
          double newValue=getSegmentNextValue(segmentIndex, k);
		  double minDis;
		  if (k==MinK)
		  {
			  minDis=100000;
         ...
         { 
			 minDis=DisArray[i][j]+0.0000000001; 
		  }
		  int startPosition=-1; 
		  ...
		  EndP[i][j]=startPosition+k;   // starting from position 1, since 0 will cause infinity
		  // compute DisArrayNext
		  newIndex=i-i/(DIMENSION-k+1); // the index in the k+1 disarray which overlaps with this one
		 if(k<MaximalK)
		 {
		  if ((i+1)%(DIMENSION-k+1)!=0) // not the last segment in each training example
		  {
			if (startPosition+k<DIMENSION)
			{  
				DisArrayNext[newIndex][j]=minDis+(newValue-training[j][startPosition+k])*(newValue-training[j][startPosition+k]); // this is the squred min-dis
				//DisArrayNext[newIndex][j]=FLT_MAX;
			}
			else
			{DisArrayNext[newIndex][j]=FLT_MAX;}
			//cout<<"\n DisArrayNext[i][j] "<<i<<","<<j<<"="<<DisArrayNext[newIndex][j];
		  }
		 }
...
// get the next value of the current Segment, if do not have the next value, return the Max_FLO
double  getSegmentNextValue(int index, int k) // index starting from 1 to consistent with the matlab version
{
   // compute the starting index and return the index's segment
    if (k<=0 || k>DIMENSION)
      return -1; 
    else
    {
      ...
      //cout<<"\n segment "<<index<<" of length "<<k<<"\n"; 
       if (column+k<DIMENSION)
	   { value=training[row][column+k];}

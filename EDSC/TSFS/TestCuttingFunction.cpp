...
Feature * ThresholdLearningKDE(int index, int k, int targetClass, double ** DisA, double RecallThreshold, double ProbalityThreshold);
...
int main()
{
   ...
   int k=1;
   for (k=MaximalK;k<=MaximalK;k++)  // k starts from 1
   { // start of outer for
	...
	Feature * testFeature=ThresholdLearningAll(238, boundThrehold, k, classIndex,  DisA, recallThreshold) ;
	PrintFeature(testFeature);
	Feature * testFeature1=ThresholdLearningKDE(238, k, classIndex, DisA, recallThreshold, probablityThreshold);
	PrintFeature(testFeature1);
 //   // compute 

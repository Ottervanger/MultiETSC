...
void classificationAllClasses(vector<Feature *> &Fs);
...
int main()
{
	...
	int option=2; // option =1 using thresholdAll, option=2, using thresholdFirst
   ...
   {  
	   ...
	   { // start of outer for
		...
		{  
			...
			{
			  temp=ThresholdLearningKDE(offset+s, k, classIndex, DisA, recallThreshold, probablityThreshold);
			}
			else
			{
				cout<<"\n invalide option";

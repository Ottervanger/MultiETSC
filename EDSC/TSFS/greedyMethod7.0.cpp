...
// in this version, ranked by earliness sctore, and classify by the default 1nn classifier. 
#include <fstream>
#include <iostream>
#include <sstream>
#include <string>
...
int main()
{
    ...
    int MaximalK=DIMENSION/3;  // maximal length
	int MinK=5;
   ...
   { // start of outer for
	...
	if ( !inputFile || !inputFile2)
	{
        ...
        DisArray(classIndex, k, DisA, EndP, NumberOfSegments, ROWTRAINING);
  ...
  int classofset=ClassIndexes[targetClass]*(DIMENSION-k+1)+1;
   ...
   if (currentf->threshold>0)
   {
      ...
      { 
	   ...
	   currentf->earlyFScore=ComputeFScore(weightedRecall/ClassNumber[targetClass], currentf->precision);

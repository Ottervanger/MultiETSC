...
Feature * ThresholdLearningAll(int index, double m, int k, int targetClass, double ** DisA, double PrecisionThresold);
...
Feature * ThresholdLearningKDE(int index, int k, int targetClass, double ** DisA, double PrecisionThreshold, double RecallThreshold);
...
int main()
{
    ...
    int MaximalK=40;  // maximal length
    int classIndex=0;    // pick the class
	//double frequentThreshold=0;
	double PrecisionThresold=0.8;   // define the precision threshold
   ...
   { // start of outer for
	...
	//Feature * testFeature=ThresholdLearningAll(232, 3, k, 0,  DisA, 0) ;
	//PrintFeature(testFeature);
	//testFeature=ThresholdLearningFirst(232, 2.98, k, 0,  DisA, 0) ;
	//PrintFeature(testFeature);
    ...
    {  
		...
		if (option==1)
		{
			 temp=ThresholdLearningAll(offset+s, boundThrehold, k, classIndex, DisA, PrecisionThresold);
		}
		else if (option==2)
		{
            ...
            {    
               ...
               { 
                  ...
                  {   
                      ...
                      {  //cout<<"\n Instance "<<i<<"("<<labelTesting[i]<<") classified by segment "<<Fs.at(f)->segmentIndex<<"of length "<<Fs.at(f)->length<< "at ending position "<<j;
                         matched=1;
...
Feature * ThresholdLearningAll(int index, double m, int k, int targetClass, double ** DisA, double PrecisionThreshold)  // this index starting from 1, m is the parameter in the bound, k is the length of feature
{

...
Feature * ThresholdLearningAll(int index, int m, int k, int targetClass, double ** DisA, double PrecisionThresold);
...
int main()
{
    ...
    int classIndex=1;    // pick the class
	//double frequentThreshold=0;
	double PrecisionThresold=0.9;   // define the precision threshold
   ...
   { // start of outer for
    ...
    {Feature * temp=ThresholdLearningAll(offset+s, boundThrehold, k, classIndex, DisA, PrecisionThresold);
       if (temp!=NULL)
       { fSet.push_back(temp);}
    }
...
Feature * ThresholdLearningAll(int index, int m, int k, int targetClass, double ** DisA, double PrecisionThreshold)  // this index starting from 1, m is the parameter in the bound, k is the length of feature
{
...
void LoadData(const char * fileName, double Data[][DIMENSION], double Labels[],int len  )
{
	...
	if ( !inputFile )
	{
		cerr << "Error reading \"" << fileName << "\": file could not be opened." << endl;
		exit(1);
	} // end if

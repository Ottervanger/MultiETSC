function [ maximum ] = max_box_constraints_f_problem( sigma, x_k, m_u, R , mu_k, mu_u, delta )
%This function computes the maximum f(x) score for the Naive Bayes box
%constraint.  Solves the problem:

%maximize (x-mu)'sigma^(-1)(x-mu)
...
%% First, set x_u for the positive elements
...
x_u(~add_elements & pos_elements') = lower_x(~add_elements & pos_elements');
    
%% Now, set x_u for the negative elements
in_elements = and(lower_x < b', upper_x > b');
x_u(in_elements & neg_elements') = (b(in_elements & neg_elements'))';
...
x_u(sub_elements & neg_elements') = lower_x(sub_elements & neg_elements');

maximum = (x_u' - b)'*S_uu*(x_u' - b) + c;

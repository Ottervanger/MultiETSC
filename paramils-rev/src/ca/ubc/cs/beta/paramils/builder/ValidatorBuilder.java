...
import ca.ubc.cs.beta.paramils.configurator.SOValidator;
...
import ca.ubc.cs.beta.paramils.configurator.MOValidator;
...
public class ValidatorBuilder {
  ...
  public AbstractAlgorithmFramework getAutomaticConfigurator(AlgorithmExecutionConfiguration execConfig, InstanceListWithSeeds trainingILWS, ParamILSOptions options,Map<String, AbstractOptions> taeOptions, String outputDir, SeedableRandomPool pool) {
    ...
    AbstractAlgorithmFramework validator;
    if (options.multiObjective)
      validator = new MOValidator(options, execConfig, instances, acTae, configSpace, instanceSeedGen, initialIncumbentList, eventManager, rh, pool, termCond, cpuTime);
    else
      validator = new SOValidator(options, execConfig, instances, acTae, configSpace, instanceSeedGen, initialIncumbent, eventManager, rh, pool, termCond, cpuTime);

    ConfigWriter cw = new ConfigWriter(outputDir + File.separator + "valid-configs" + options.seedOptions.numRun);
    validator.setupConfigWriter(cw);

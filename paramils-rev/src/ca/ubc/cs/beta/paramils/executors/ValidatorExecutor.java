...
import ca.ubc.cs.beta.paramils.builder.ValidatorBuilder;
...
public class ValidatorExecutor {
  ...
  public static int oldMain(String[] args) {
    ...
    try {
      ...
      AbstractAlgorithmFramework paramils = validatorBuilder.getAutomaticConfigurator(execConfig, trainingILWS, options, taeOptions, outputDir, pool);
      ...
      validatorBuilder.getEventManager().shutdown();
  ...
  private static ParamILSOptions parseCLIOptions(String[] args) throws ParameterException, IOException {
    ...
    try {
      try {
        ...
        ValidatorExecutor.logLocation = options.logOptions.getLogLocation(outputDir,options.seedOptions.numRun);
      ...
      JCommanderHelper.logCallString(args, "validate");
